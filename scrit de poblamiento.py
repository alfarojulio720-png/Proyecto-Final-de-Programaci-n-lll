...
def main():
    ...
    cur.executemany(
        'INSERT INTO locales (id_empresa, nombre_local, direccion, latitud, longitud) VALUES (%s,%s,%s,%s,%s)',
        locales
    )
    conn.commit()

    # -----------------------------
    # IDs de los locales insertados
    # -----------------------------
    cur.execute('SELECT id_local FROM locales')
    local_ids = [r[0] for r in cur.fetchall()]

    # -----------------------------
    # INSERTAR MOVIMIENTOS 12 MESES
    # -----------------------------
    ...
    cur.executemany(
        'INSERT INTO movimientos_mensuales (id_local, mes, anio, ventas, compras) VALUES (%s,%s,%s,%s,%s)',
        movimientos
    )
    conn.commit()
